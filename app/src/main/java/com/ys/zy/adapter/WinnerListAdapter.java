...
public class WinnerListAdapter extends CommonAdapter<WinnerUserBean.DataBeanX.DataBean> {
    ...
    public void convert(ViewHolder helper, WinnerUserBean.DataBeanX.DataBean item, int position) {
        if (StringUtil.isBlank(item.consumer_img)) {
            Glide.with(mContext).load(R.mipmap.bg_default_head2).into((CircleImageView) helper.getView(R.id.iv_));
        } else {
            Glide.with(mContext).load(FunctionApi.getImagePath(item.consumer_img)).into((ImageView) helper.getView(R.id.iv_));
        }
