...
public class CircleProgressBar extends View {
    ...
    private int text1Size = 20;//sp
    private int text2Size = 12;//sp
    private String text1Color = "#000000";
    private String text2Color = "#000000";
    private double text2ScaleY = 0.5;//text2 y的高度
...
//        circlePaint.setStrokeCap(Paint.Cap.ROUND); // 把每段圆弧改成圆角的
    ...
    private void drawText(Canvas canvas, int center) {
        ...
        textPaint.setColor(Color.parseColor(text1Color)); // 设置文字颜色
        textPaint.setTextSize(DensityUtil.sp2px(getContext(), text1Size)); // 设置要绘制的文字大小
     ...
     * @param
     */
    private void drawText2(Canvas canvas, int center) {
        float result = (currentValue * 100.0f / maxValue * 1.0f); // 计算进度
        if (StringUtil.isBlank(description)) {
            description = String.format("%.1f", result) + "%";
        }
        textPaint.setTextAlign(Paint.Align.CENTER); // 设置文字居中，文字的x坐标要注意
        textPaint.setColor(Color.parseColor(text2Color)); // 设置文字颜色
        textPaint.setTextSize(DensityUtil.sp2px(getContext(), text2Size)); // 设置要绘制的文字大小
        textPaint.setStrokeWidth(0); // 注意此处一定要重新设置宽度为0,否则绘制的文字会重叠
        Rect bounds = new Rect(); // 文字边框
        textPaint.getTextBounds(description, 0, description.length(), bounds); // 获得绘制文字的边界矩形
        Paint.FontMetricsInt fontMetrics = textPaint.getFontMetricsInt(); // 获取绘制Text时的四条线
        int baseline = (int) (center * text2ScaleY) + circleWidth / 2 + (fontMetrics.bottom - fontMetrics.top) / 2 - fontMetrics.bottom; // 计算文字的基线,
        // 方法见http://blog.csdn.net/harvic880925/article/details/50423762
        canvas.drawText(description, center, baseline, textPaint); // 绘制表示进度的文字
    }
    ...
    public double getText2ScaleY() {
        return text2ScaleY;
    }

    public void setText2ScaleY(double text2ScaleY) {
        this.text2ScaleY = text2ScaleY;
    }
}
